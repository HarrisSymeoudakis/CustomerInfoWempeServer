...
app.get("/swagger/CustomerUserFields", async (req, res) => {
  try {
    if (!customerId) {
      return res
        .status(400)
        .send("customerId not available. Ensure the webhook has been called.");
    }
...
app.get("/swagger/customerOrders", async (req, res) => {
  try {
    ...
    const baseUrl =
      "https://90571062-test-retail-ondemand.cegid.cloud/Y2/90571062_002_TEST/api/customer-documents/v1";
...
app.get("/PostBackExtForm/redirect", (req, res) => {
  ...
  res.redirect(
    `https://retail-services.cegid.cloud/et/pos/pulsarcustomer/externalcustomer/callback?sessionId=${sessionId}`
  );
});
